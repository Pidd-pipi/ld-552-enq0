...
async function main() {
  ...
  await prisma.offer.create({ data: { candidateId: c1.id, jobId: job1.id, salary: '42000', startDate: new Date(Date.now() + 1000 * 60 * 60 * 24 * 30), status: OfferStatus.DRAFT } });
  await prisma.offer.create({ data: { candidateId: c2.id, jobId: job1.id, salary: '20000', startDate: new Date(Date.now() + 1000 * 60 * 60 * 24 * 45), status: OfferStatus.DRAFT } });
