...
export class CandidatesService {
  ...
  findOne(id: number) { return this.prisma.candidate.findUnique({ where: { id }, include: { resumes: { include: { job: true, interviews: { include: { interviewer: { select: publicUserSelect } } } } }, offers: { include: { job: true, approver: { select: publicUserSelect }, approvals: { orderBy: { level: 'asc' }, include: { decider: { select: publicUserSelect } } } } } } }); }
  ...
  offers(id: number) { return this.prisma.offer.findMany({ where: { candidateId: id }, include: { job: true, approver: { select: publicUserSelect }, approvals: { orderBy: { level: 'asc' }, include: { decider: { select: publicUserSelect } } } } }); }
}
